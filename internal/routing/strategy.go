// strategy.go 是 策略框架：Strategy 对候选列表做
// 稳定排序，产出尝试顺序。与弹性层分工：隔离/配额阻断是硬边界（先短路），
// 策略只对幸存者排序；打分路由即默认的 auto 策略。
//
...
// RouteContext 携带跨层路由输入：v1 只有策略名；
// 随后接入压缩后 token 数（PromptTokens）， 复杂度分数随后，
// quality 策略（2026-08-29）起携带目标模型名（能力分查询键）。
type RouteContext struct {
	StrategyName string
	// PromptTokens 是 L4 压缩管线输出的请求粗估 token（跨层协同：
	// 压缩后 token 参与候选筛选/打分，）。0=未知，不过滤。
	PromptTokens int64
...
//	"@combo:free-tier" → ("combo", "free-tier", nil) // rest 是组合名
//	"@bogus:x" → 错误（未知指令名）
...
type dispatchConfig struct {
	strategyName   string
	sessionID      string // sticky session
	promptTokens   int64  // 压缩后 token（跨层输入；0=未知）
	comboName      string // 命名模型组（；空=普通分发）
	pinnedProvider string // 全局路由钉选（；空=未钉）
	targetProvider string // 定向分发 provider（Fusion；空=普通路径）
	targetModel    string // 定向分发模型（与 targetProvider 成对）
	smart          bool   // ML 路由（"@smart"；候选=Smart 计划成员）
	qualityAuto    bool   // 裸 "@quality"（自动选最强：按 BestModel 排序并逐家改写模型）
}

// WithStrategyName 覆盖本次分发的策略（默认 auto = 打分排序）。
func WithStrategyName(name string) DispatchOption {
	return func(c *dispatchConfig) { c.strategyName = name }
}

// WithCombo 选择命名模型组：候选=组合成员（声明序），逐尝试
...
// WithTarget 定向分发（Fusion 扇出/合成原语）：跳过候选选择与
...
// strategyByName 解析策略名；auto/空 → nil（走 rank）。
...
// ordered 返回本次分发的候选顺序：策略排序> 打分排序
// > 注册序。Scoring 未启用时 rank() 退化为注册序。tokens 是
