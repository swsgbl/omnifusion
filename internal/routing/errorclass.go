// errorclass.go 是 M2.1 错误分类器（docs/04 §4.4"错误即数据"）：
...
// ErrorKind 归一化错误类别。前六类对应 docs/04 §4.4 的六类错误；
...
const (
	// RateLimitCooldown 是 rate_limit 的 Connection 冷却时长（M2.2 消费）。
...
// Policy 是某类错误的处理策略，字段对应 docs/04 §4.4 处理策略表。
// M2.1 只产出策略数据；冷却/锁定/降分的执行在 M2.2 状态机接入。
type Policy struct {
	// Failover 表示是否切换下一候选。全部类别默认允许（保持 M1 行为）。
	Failover bool
	// Cooldown 是 Connection 层冷却时长；0 表示不冷却。
	Cooldown time.Duration
	// LockoutModel 表示把该 Model 锁定到额度重置点（reset-aware，M2.2）。
	LockoutModel bool
	// InvalidKey 表示把关联 API key 置为 invalid，需人工处理。
	InvalidKey bool
	// HealthPenalty 表示记一次健康降分（M2.4 打分路由消费）。
	HealthPenalty bool
}

// policyTable 是 docs/04 §4.4 处理策略表的代码形态：
//   - rate_limit 冷却 Connection 30s；
//   - quota_exhausted 锁定 Model（不冷却连接——连接本身没病）；
//   - auth_invalid 长冷却 + 置 invalid；
//   - upstream_5xx / stream_broken 降分但不冷却（仅同元组退避，M2.2）；
...
// classifyUpstream 按 docs/04 §4.4 的状态码映射分类上游错误。
func classifyUpstream(ue *provider.UpstreamError) ErrorKind {
	switch {
	case ue.Status == 0:
		// M1.5 契约：流内嵌 {"error":...} 事件归一为 Status==0。
		return KindStreamBroken
