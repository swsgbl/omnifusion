...
type Server struct {
	...
	catalog      *routing.Catalog                 // 模型目录（M3.5；nil = /v1/models 给空表）
	cache        *intelligence.SemCache           // 语义缓存（M4.6；nil = 不查不写）
	fusion       *intelligence.FusionRunner       // Fusion 扇出合成（M6.1；nil = @fusion 边界 400）
	memory       *intelligence.Memory             // FTS5 会话记忆（M6.4；nil = opt-in 头无效果）
	comboPipes   map[string]*compression.Pipeline // 组合压缩管线（M4.7；键存在值可 nil=纯路由组合）
	keySources   map[string]string                // key 来源描述（M4.8 Dashboard；provider → stored/env:VAR/none/-）
	mcpHandler   http.Handler                     // MCP Streamable HTTP（M5.1；nil = 不挂 /mcp）
	guard        *security.Guardrails             // 规则型护栏（M5.4；nil = 未启用）
	metrics      *obs.Metrics                     // Prometheus 指标（M5.5；nil = 未启用，全 no-op）
	gatewayToken string

	a2aCard  *a2a.AgentCard // A2A 发现清单（M7.2；nil = 不挂 agent-card 与 /rpc）
	a2aModel string         // A2A 缺省目标模型（可含 @指令）

	pinMu    sync.Mutex             // 全局路由钉选（M5.2）
	pinName  string                 // 钉选 provider；空 = 未钉
	pinUntil time.Time              // 钉选到期时刻（TTL，默认 30m）
	defCombo atomic.Pointer[string] // 默认压缩组合（M5.2；nil/空 = 未设）
	cstats   comboStats             // 压缩统计聚合（M5.6；零值可用，恒装配）
}
...
func (s *Server) Handler() http.Handler {
	...
	mux.Handle("POST /v1/messages", // Anthropic 入站（M3.1，Claude Code 直连）
		s.requireAnthropicKey(http.HandlerFunc(s.handleMessages)))
	mux.Handle("POST /v1beta/models/", // Gemini 入站（M3.2，通配符后不能跟字面后缀，handler 内拆路径）
		s.requireGeminiKey(http.HandlerFunc(s.handleGeminiGenerateContent)))
	mux.HandleFunc("GET /dashboard", s.handleDashboardRoot) // M4.8：落到默认页（透传 ?key=）
	mux.Handle("/dashboard/api/",                           // M5.2 控制面 JSON API：外层挡伪造 token，内层逐端点 requireScope（control_api.go）
		s.requireAnyScope(http.HandlerFunc(s.handleDashboardAPI)))
	mux.Handle("/dashboard/", // Dashboard HTML 页面（M4.8，master key；与 api/ 同为前缀模式，方法在 handler 内收敛）
		s.requireDashboardKey(http.HandlerFunc(s.handleDashboard)))
	if s.mcpHandler != nil { // MCP Streamable HTTP（M5.1）：Claude Code 等 MCP 客户端入口
		mux.Handle("/mcp", s.requireAnyScope(s.mcpHandler)) // M5.2：scoped token 亦可入（工具可见性按 scope 收敛）
	}
	if s.metrics != nil { // Prometheus 指标（M5.5）：网关 key 鉴权，不新开匿名信息面
		mux.Handle("GET /metrics", s.requireGatewayKey(s.metrics.Handler()))
	}
	if s.a2aCard != nil { // A2A v1.0（M7.2）：发现清单公开；/rpc 强制网关 key
