// combo.go 承载 M4.7 组合路径的 server 侧编排：已知组合名判定 +
// per-path 压缩策略应用（路由组合绑定的压缩组合在此执行，docs/04
// §3 管线序 L4 压缩 → L3 路由）。
package server
...
// WithPromptTokens（压缩后 token 喂给 L3 窗口过滤，M4.5 通道）。
...
func (s *Server) comboCompress(r *http.Request, req *schema.UnifiedRequest, combo string) []routing.DispatchOption {
	...
	req.Messages = out
	s.cstats.record(combo, int64(before), int64(after), stats) // M5.6 压缩统计
	if s.log != nil {
