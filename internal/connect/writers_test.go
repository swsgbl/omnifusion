// writers_test.go 承载五家 CLI 写入器的单元测试（与 internal/connect
// 实现同包，可测未导出函数）。
package connect
...
// TestApplyClaudeMergeAndDisconnect 走 applyClaude 全链路：合并保留
// 已有 env 键、备份生成、disconnect 精确移除我们写的两个键。
func TestApplyClaudeMergeAndDisconnect(t *testing.T) {
	dir := t.TempDir()
...
// TestApplyCodexTopKeyBeforeTables TOML 顶层键必须在表头之前；
// 二次 connect 幂等（不重复堆块）。
func TestApplyCodexTopKeyBeforeTables(t *testing.T) {
	// 钉住持久化钩子：测试绝不写真实用户环境（Windows setx / unix rc）。
	orig := persistEnvVar
	persistEnvVar = func(key, value string) (string, error) {
		return "（测试：跳过持久化 " + key + "）", nil
	}
	t.Cleanup(func() { persistEnvVar = orig })
...
// TestApplyGeminiOverrideNotice 覆盖既有 GEMINI_API_KEY 时必须给出
// 提示；--print 不落盘。
func TestApplyGeminiOverrideNotice(t *testing.T) {
	dir := t.TempDir()
